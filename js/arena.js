...
// Helper Functions — must be defined first (used by arenaScreen, arenaData, etc.)
function calcBaseDamage(power, atk, def) { return Math.max(1, power + atk - def); }
function hasStatus(c, kind) { return c.statuses && c.statuses.some(s => s.kind === kind); }
function addStatus(c, statusObj) { if (!c.statuses) c.statuses = []; c.statuses.push(statusObj); }
...
function applyDamage(actor, foe, amt) {
  ...
  if (finalDmg > 0) foe.charge = Math.min(100, (foe.charge || 0) + 10);
...
window.calcBaseDamage = calcBaseDamage;
window.addStatus = addStatus;
window.consumeStatus = consumeStatus;
window.tickStatusesEndTurn = tickStatusesEndTurn;
